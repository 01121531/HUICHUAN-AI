...
- [构建与验证](#构建与验证)
- [升级说明](#升级说明)
- [v1.0.4 更新摘要](#v104-更新摘要)
- [v1.0.3 更新摘要](#v103-更新摘要)
...
- 修复 Linux/systemd 单节点部署在线升级卡在 94% “正在重启服务”的问题。
- Unix helper 改为先备份并替换二进制，再等待旧进程退出，避免被 systemd `Restart=always` 立即拉起旧版本。
- 增加 systemd 环境检测，优先等待服务管理器拉起新版本，避免额外启动第二个进程。
- 增强 Linux 进程退出判定，识别 zombie 进程，降低重启阶段误等待风险。

## v1.0.3 更新摘要

- 在线升级扩展为支持 Windows、Linux、macOS 单节点独立二进制部署。
- Release 资产匹配改为按当前 GOOS/GOARCH 选择平台包和 checksum 文件。
- 新增 Linux/macOS update helper，支持替换、重启健康检查和失败回滚。
- Release 工作流增加 Linux/macOS 多架构在线升级资产。
- 更新系统设置页面的在线升级能力说明和多语言文案。
