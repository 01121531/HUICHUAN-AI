...
| 项目 | 值 |
| --- | --- |
| 版本 | [`v1.1.0`](https://github.com/01121531/HUICHUAN-AI/releases/tag/v1.1.0) |
...
- 完善数据快照详情页多语言显示，补齐对话、采集状态、推理状态、角色、工具调用和停止原因等文案。
- 将原始协议值转换为可读名称，例如 `openai-responses` 显示为 `OpenAI Responses`。
- 补齐推理内容采集策略、保存模式和采样比例的七种语言文案，避免缺失翻译时回退为英文。
- 使用日志计价详情移除倍率列和分组倍率卡片，直接展示请求发生时的有效单价。
- 有效单价仅在前端使用计价快照中的基础单价和冻结倍率进行展示换算，不修改后端计费、额度结算或历史快照数据。
- 优化计价单位显示，`1000000` 显示为 `/M`、`1000` 显示为 `/K`，按次费用显示为 `/request` 或 `/call`。
