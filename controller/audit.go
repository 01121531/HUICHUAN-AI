...
var auditContentTemplates = map[string]string{
	...
	"usage_log.ip_reveal":       "Revealed full request IP values for ${record_count} usage logs (${scope})",
	"usage_log.export":          "Exported ${record_count} usage logs as ${format} (${selection_mode}, ${ip_visibility} IP)",
	"usage_log.export_create":   "Created usage log export task ${task_id} for ${record_count} records",
	"usage_log.export_download": "Downloaded ${record_count} usage logs as ${format} (${ip_visibility} IP)",
	"usage_log.export_failed":   "Usage log export task ${task_id} failed (${error_code})",
